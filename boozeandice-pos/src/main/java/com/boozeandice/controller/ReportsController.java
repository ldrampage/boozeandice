...
public class ReportsController {
	...
	public String reportsPage(Model model, @RequestParam(name="zdate", required=false) String zdate) throws Exception {
			...
			for(TransactionItem transactionItem : transaction.getTransactionItem()) {
				for(ProductCategory productCategory : productCategoryList ) {
					if(productCategory.equals(transactionItem.getProduct().getProductCategory())) {
						productCategory.setSoldCount(productCategory.getSoldCount() + (1 * transactionItem.getQuantity()));
						productCategory.setQuantityNetSales(productCategory.getQuantityNetSales() + (transactionItem.getProduct().getPrice() * transactionItem.getQuantity()));
					}
				}
		...
		for(Transaction transaction : transactionListPaid) {
			...
			if(transaction.getCardBrand() != null && CardBrand.AMEX.getDescription().equals(transaction.getCardBrand())) {
				pmpbd.setAmex(pmpbd.getAmex() + transaction.getTotal());
				pmpbd.setTotal(pmpbd.getTotal() + transaction.getTotal());
			}
			
			if(transaction.getCardBrand() != null && CardBrand.VISA.getDescription().equals(transaction.getCardBrand())) {
				pmpbd.setVisa(pmpbd.getVisa() + transaction.getTotal());
				pmpbd.setTotal(pmpbd.getTotal() + transaction.getTotal());
			}
			
			if(transaction.getCardBrand() != null && CardBrand.MASTERCARD.getDescription().equals(transaction.getCardBrand())) {
